...
const labels: Record<ConnectionState, string> = {
  synced: 'Conectado · datos al día',
  offline: 'Sin conexión',
  retrying: 'Reintentando conexión…',
};

export function ConnectionBanner({ state = 'synced' }: { state?: ConnectionState }): React.JSX.Element {
  const tone = tones[state];
  const icon =
    state === 'synced' ? ('checkmark' as const) : state === 'offline' ? ('cloud-offline' as const) : ('refresh' as const);
