import { Pressable, type StyleProp, StyleSheet, Text, type ViewStyle, ActivityIndicator } from 'react-native';
...
const palette: Record<Variant, { bg: string; fg: string; pressed: string }> = {
  primary: { bg: colors.primary, fg: colors.onPrimary, pressed: colors.primaryHover },
  secondary: { bg: colors.surface, fg: colors.ink, pressed: colors.surfaceSunken },
  ghost: { bg: 'transparent', fg: colors.primary, pressed: colors.primarySoft },
  danger: { bg: colors.surface, fg: colors.statusDanger, pressed: colors.statusDangerSoft },
};
...
export function Button({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.base,
        { backgroundColor: tones.bg, borderColor: variant === 'secondary' ? colors.borderStrong : 'transparent' },
        isSm && styles.md,
        !isSm && styles.lg,
      ...
      {loading ? (
        <ActivityIndicator color={variant === 'secondary' || variant === 'ghost' || variant === 'danger' ? tones.fg : colors.onPrimary} />
      ) : (
